...
// 資料庫查詢
$sql = "SELECT CONCAT(DATE_FORMAT(量測時間, '%H'), ':', LPAD((MINUTE(量測時間) DIV 2) * 2, 2, '0')) AS minute_interval, AVG(體溫) AS avg_temperature, AVG(血氧) AS avg_blood_oxygen, AVG(心率) AS avg_heart_rate FROM 健康數據 GROUP BY minute_interval ORDER BY minute_interval ASC";
