...
firebaseRefHeartRate.on("value", function(snapshot) {
    ...
    if (heartRate < 40 || heartRate > 120) {
        heartRateElement.classList.add("abnormal");
    } else {
        heartRateElement.classList.remove("abnormal");
    }
});
...
firebaseRefTemp.on("value", function(snapshot) {
    ...
    if (temp < 35.5 || temp > 37.5) {
        tempElement.classList.add("abnormal");
    } else {
        tempElement.classList.remove("abnormal");
    }
});
...
firebaseRefSpo2.on("value", function(snapshot) {
    ...
    if (spo2 < 95) {
        spo2Element.classList.add("abnormal");
    } else {
        spo2Element.classList.remove("abnormal");
    }
});
